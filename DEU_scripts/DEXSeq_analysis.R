...
option_list = list(
  make_option(c("-i", "--countfolder"), type="character", default=NULL,
              help="path to folder of counts", metavar="character"),
  make_option(c("-o", "--resultfolder"), type="character", default=NULL,
...
normedcount_PATH = paste(opt$resultfolder, paste(paste(epi_id1, epi_id2, sep='_'), "normedcount.csv", sep='_'), sep='/')
write.table(dxd.count, normedcount_PATH, quote=FALSE, sep="\t", dec=".", row.names=FALSE, col.names=TRUE)

# cat("\n---> Saving DEXSeq result", append = TRUE)
print("Saving DEXSeq result")
r_data_PATH = paste(opt$resultfolder, paste(paste(epi_id1, epi_id2, sep='_'), "RData", sep='.'), sep='/')
save(dxd.res, file=r_data_PATH)

result_PATH = paste(opt$resultfolder, paste(paste(epi_id1, epi_id2, sep='_'), "res.csv", sep='_'), sep='/')
...
html_PATH = paste(opt$resultfolder, paste(paste(epi_id1, epi_id2, sep='_'), "html", sep='_'), sep='/')
DEXSeqHTML(dxd.res, 
          path = html_PATH,
