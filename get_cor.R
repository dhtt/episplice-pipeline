...
get_all_pairs.his <- function(all_pairs.his, his) {
  ...
  for (i in 1:length(all_pairs.his)) {
    ...
    pair.his <- pair.his %>%
      dplyr::mutate(
        gene = id$gene, exon = paste("E", id$exon, sep=''), type = V3,
...
common_id = paste(all_pairs.his_list[[1]]$gene_id, all_pairs.his_list[[1]]$exon_id, sep = '_')
all_pairs.exp$filter_id = paste(all_pairs.exp$gene_id, all_pairs.exp$exon_id, sep = '_')
all_pairs.exp = all_pairs.exp[all_pairs.exp$filter_id %in% common_id,] %>% dplyr::select(-filter_id)
print(paste("Check if exp_id == his_id:", table(all_pairs.exp$gene_id == all_pairs.his_list[[1]]$gene_id)))
...
temp1 = readRDS("/home/dhthutrang/Krebs/Shen/cor_analysis/all_pairs.exp.RDS")
temp2 = readRDS("/home/dhthutrang/Krebs/Shen/cor_analysis/all_pairs.his_list.RDS")
temp2 = temp2$H3K4me3
head(temp2[[1]])

temp1[temp1$gene_id == "S100A2",]
temp2[[1]][temp2[[1]]$gene_id == "S100A2",]
cor(temp2[[3]][temp2[[3]]$gene_id == "TBC1D4", 3], temp1[temp1$gene_id == "TBC1D4", 3])
...
r_val$H3K4me3[r_val$H3K4me3$gene_id == "S100A2", ]
r_val
# sapply(r_val, dim)
...
temp = intersect(p_val[[2]]$gene_id, r_val[[2]]$gene_id)
temp
paste(temp, collapse = ', ')
