...
sys.path.append("/home/dhthutrang/episplice-pipeline/utils")
...
def execute_workflow(args=None):
    ...
    for file in listdir(merged_library_path):
        if file.endswith('sorted.bam'):
            ...
            subprocess.call(
                ...
                %(merged_library_path, alignment_path),
                shell=True
            )
            subprocess.call(
                "python utils/sam2bed.py -i %s -o %s"
                %(join(alignment_path, file_name_sam), join(alignment_path, file_name_bed)),
                shell=True
            )
    ...
    for histone_type in histone_types:
        sample_ids = treatment_ids + [control_id]
        for sample_id in sample_ids:
            file_id = '_'.join([sample_id, histone_type])    
            alignment_files = [file for file in listdir(alignment_path) if file.startswith(file_id) & file.endswith('.bed')]
            merge_bed_file(input_dir=alignment_path, output_dir=alignment_path, 
    ...
    for dirpath, dirs, files in os.walk(manorm_output):	 
        for file in files:
            if file.endswith(".xls"):
                ...
                annotated_file_path = join(manorm_output_annotated, '_'.join(file.split('_')[:-2]) + ".bed")
